...
- `MATRIX_BRIDGE_FEISHU_PROVISIONING_READ_TOKEN`：只读接口。
- `MATRIX_BRIDGE_FEISHU_PROVISIONING_WRITE_TOKEN`：写入接口（如创建、回放）。
- `MATRIX_BRIDGE_FEISHU_PROVISIONING_DELETE_TOKEN`：删除/高风险接口。
- 向后兼容：`MATRIX_BRIDGE_FEISHU_PROVISIONING_TOKEN` 与 `MATRIX_BRIDGE_FEISHU_PROVISIONING_ADMIN_TOKEN` 仍可作为回退。
