...
- **文件/媒体共享**（图片、视频、文档等）
- **用户与房间映射同步**
- 基于飞书 API 的 **长连接事件集成**（兼容 webhook 模式）
- Rust 实现，具备**高性能与稳定性**
...
- **Feishu Integration**：飞书 API 客户端、长连接事件流与 webhook 兼容处理
...
- **Bridge Settings**：飞书应用凭据与事件接入模式（`long_connection` / `webhook`）
- **Callback Security Settings（Webhook 模式）**：`listen_secret`、可选 `encrypt_key` 与 `verification_token`
...
2. **配置事件订阅（推荐：长连接模式）**
   - 在飞书开放平台选择长连接模式
   - 长连接模式无需公网 Request URL
   - 订阅必需事件（如 `im.message.receive_v1`、`im.message.recalled_v1`、群成员/群生命周期事件）

3. **可选：Webhook 兼容模式**
   - 设置 `bridge.event_mode: "webhook"`
   - 回调地址设为 `http://your-server:8081/webhook`
   - 配置回调校验（`listen_secret`，可选 `encrypt_key + verification_token`）
...
桥接服务启动后，可直接在 Matrix 房间里绑定飞书群：

1. 创建（或选择）一个 Matrix 房间。
2. 邀请桥接机器人进房间：
   - 格式：`@<appservice.bot.username>:<homeserver.domain>`
   - 示例：`@feishubot:localhost`
3. 在房间里执行命令：
   - `!feishu help` 查看可用命令
   - `!feishu bridge <feishu_chat_id>` 绑定当前房间到飞书群
4. 验证映射是否生效：
   - `./target/release/matrix-bridge-feishu -c config.yaml mappings`
   - 或查询 `GET /admin/mappings`
5. 需要解除绑定时：
   - `!feishu unbridge`

说明：
- 命令前缀固定为 `!feishu`。
- 飞书群 `chat_id` 常见格式为 `oc_xxxxxxx`。
...
services:
  matrix-bridge-feishu:
    ...
    ports:
      - "8080:8080"
      # 仅在 bridge.event_mode=webhook 时需要
      # - "8081:8081"
    volumes:
...
内置脚本用于评估 webhook 突发与队列稳定性（仅 webhook 模式）：
...
2. **鉴权失败**：核对飞书应用凭据
3. **数据库异常**：检查数据库连接与权限
4. **消息未桥接**：优先检查长连接状态与日志；若 webhook 模式则核对 webhook 配置与地址
5. **加密错误**：核对 webhook 的 encrypt key 与 verification token
...
| webhook `invalid signature` / `missing signature`（webhook 模式） | 回调签名不一致 | 核对 `listen_secret` 与 `X-Lark-*` 头透传 |
...
- **Webhook 签名失败（webhook 模式）**：检查 `bridge.listen_secret` 与 `X-Lark-Request-Timestamp`、`X-Lark-Request-Nonce`、`X-Lark-Signature`
...
- **回调加密**：若使用 webhook 模式，生产环境建议启用飞书回调加密
- **网络安全**：生产环境使用 HTTPS
...
|---|---|---|---|
| `im.message.receive_v1` | 已支持 | 未识别类型降级为文本 | `src/feishu/service.rs:dispatch_event_payload` |
...
| Dead-letter + replay | 已支持 | 失败飞书事件任务可查询与回放 |
...
4. 使用长连接模式（默认：`bridge.event_mode=long_connection`）
5. 若使用 webhook 模式，配置回调安全：`listen_secret`，可选 `encrypt_key + verification_token`
